...
public class SQLPersister
{
    ...
    throws Exception
  {
    ...
    while(pkli.hasNext()) {
      ...
      if (value == null) throw new Exception("Value of primary key property not set. progid='"+pm.getProgId()+"'");
    ...
    throws Exception
  {
    ...
    while(pkli.hasNext()) {
      IPropMeta pm = pkli.next();
      Object javaValue = bean.get(pm.getProgId());
      if (javaValue == null) throw new Exception("Value of primary key property not set. progid='"+pm.getProgId()+"'");
      Object jdbcValue = DataTypes.java2Jdbc(pm, javaValue);
      ps.setObject(idx1, jdbcValue, pm.getColType());
      idx1++;
    }
